	...
	*/
	...
	var max = function ( arr, func ) {
		return arr.reduce( function ( tuple, cur, index ) {
			var value = func( cur )
			return ( tuple.value >= value ) ? tuple : {
				element: cur,
				value: value
			};
		}, {
	...
	*/
	var heuristic = function ( state, maximizingPlayer ) {
		...
		function lines( player ) {
			var twoLines = state.numLines( 2, player );
			var threeLines = state.numLines( 3, player );
			var fourLines = state.numLines( 4, player );

			var stateScore = twoLines + threeLines * 5 + fourLines * 10000;
			return stateScore;
		}

		var scoreDiff = lines( maximizingPlayer ) - lines( minimizingPlayer );
	...
	*/
	var minimax = function ( state, depth, maximizingPlayer ) {
		...
		if ( currentPlayer !== maximizingPlayer ) {
			return Math.max.apply(null, possibleStates.map( nextState => minimax( nextState, depth - 1, maximizingPlayer ) ) )
		} else {
			return Math.min.apply(null, possibleStates.map( nextState => minimax( nextState, depth - 1, minimizingPlayer ) ) )
		}
